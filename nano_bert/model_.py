...
class ALBertEmbeddings(torch.nn.Module):
    def __init__(self, vocab_size, n_embed, max_seq_len, hidden_embed = 2): # n_embed = 3, max_seq_len = 16
        super().__init__()
        self.max_seq_len = max_seq_len

        self.word_embeddings = torch.nn.Embedding(vocab_size, hidden_embed) # number of words is length of text, each words has length n_embed
        self.pos_embeddings = torch.nn.Embedding(max_seq_len, hidden_embed)
...
class NanoBERT(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        positione_mbeddings = self.position(embeddings)
